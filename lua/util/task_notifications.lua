-- most retrofitted from this https://github.com/rcarriga/nvim-notify/issues/71
-- spinner frames from
-- https://github.com/ryanoasis/nerd-fonts/blob/master/assets/Mononoki/Mononoki%20Regular%20Nerd%20Font%20Complete.ttf
...
local function update_spinner(task_name, curr_wait_defer)
  ...
  M.cache[task_name].notification = vim.notify(M.cache[task_name].msg, nil, updated_notif_opts)

  vim.defer_fn(function()
    update_spinner(task_name, curr_wait_defer)
  end, 64)
end
...
function M.clear(task_name, log_level)
  ...
  if M.cache[task_name].notification ~= nil then
    clear_notification_opts.replace = M.cache[task_name].notification
    M.cache[task_name].notification = nil
  end

  vim.notify(M.cache[task_name].msg, vim.log.levels.INFO, clear_notification_opts)
...
function M.start(task_name, msg)
  ...
  M.cache[task_name].notification = vim.notify(msg, vim.log.levels.INFO, {
    title = task_name,
