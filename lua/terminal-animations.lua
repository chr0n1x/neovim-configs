...
local function animate_resize(self, target, snap_config)
  ...
  if self._resize_anim and self._resize_anim:is_running() then self._resize_anim:finish() end
  ...
  self._resize_anim = Animation(300, 60, require("animation.easing").in_out_sine, function(f)
    ...
    if f == 1 and not snapped then
      ...
      vim.api.nvim_win_set_config(win, snap_config or {
        relative = "editor", row = math.floor(target.row), col = math.floor(target.col),
        width = math.floor(target.width), height = math.floor(target.height),
        style = "minimal", border = border,
      })
      return
    end
    vim.api.nvim_win_set_config(win, {
      relative = "editor", style = "minimal", border = border,
      row = math.floor(sr + f * (target.row - sr)), col = math.floor(sc + f * (target.col - sc)),
      width = math.floor(sw + f * (target.width - sw)), height = math.floor(sh + f * (target.height - sh)),
    })
  end)
